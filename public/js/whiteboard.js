...
function updateCrew(crewResponse) {
  ...
  if (!crew.success) {
    ...
    crew.attendant.rn = checkRiderNum(
      crew.attendant.id,
      crew.attendant.rn,
      riderRadioNums
    );
    crew.observer.rn = checkRiderNum(
      crew.observer.id,
      crew.observer.rn,
      riderRadioNums
    );
    document.querySelector("#cc").innerHTML = crew.cc.name;
    document.querySelector("#cc-rn").innerHTML = cleanRadioNum(
      crew.cc.id,
      crew.cc.rn
    );
    document.querySelector("#driver").innerHTML = crew.driver.name;
    document.querySelector("#driver-rn").innerHTML = cleanRadioNum(
      crew.driver.id,
      crew.driver.rn
    );
    document.querySelector("#rider1").innerHTML = crew.attendant.name;
    document.querySelector("#rider1-rn").innerHTML = cleanRadioNum(
      crew.attendant.id,
      crew.attendant.rn
    );
    document.querySelector("#rider2").innerHTML = crew.observer.name;
    document.querySelector("#rider2-rn").innerHTML = cleanRadioNum(
      crew.observer.id,
      crew.observer.rn
    );
    document.querySelector("#dutysup").innerHTML = crew.dutysup.name;
    document.querySelector("#dutysup-rn").innerHTML = cleanRadioNum(
      crew.dutysup.id,
      crew.dutysup.rn
    );
  }
